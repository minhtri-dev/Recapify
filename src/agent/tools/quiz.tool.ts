...
import { type QuestionType, type Difficulty } from '@schemas/question.model'
...
export class QuizGeneratorTool extends Tool {
  name = 'QuizGenerator'
  description =
    'Generate educational quizzes based on user notes using similarity search and LLM generation'
  ...
  async _call(input: string): Promise<string> {
    try {
      const {
        ...
        difficulty = 'MEDIUM',
      } = JSON.parse(input) as QuizGenerationInput
      ...
      const relevantNotes = await searchSimilarNotes(
        prompt,
        Math.min(10, questionCount * 2),
      )

      if (relevantNotes.length === 0) {
        throw new Error(
          'No relevant notes found for the given prompt. Please ensure you have notes that relate to your topic.',
        )
      }
      ...
      return JSON.stringify({
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Unknown error occurred during quiz generation',
      })
    }
  }
